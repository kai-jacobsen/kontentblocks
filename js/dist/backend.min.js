...
var KB=KB||{};KB.Backbone=KB.Backbone||{},KB.Backbone.AreasCollection=Backbone.Collection.extend({});var KB=KB||{};KB.Backbone=KB.Backbone||{},KB.Backbone.ModulesCollection=Backbone.Collection.extend({}),KB.Backbone.ModulesDefinitionsCollection=Backbone.Collection.extend({setup:function(){return this.categories=this.prepareCategories(),this.sortToCategories(),this},getModules:function(id){return this.categories[id].modules},sortToCategories:function(){var that=this;_.each(this.models,function(model){if(model.get("settings").hidden!==!0){var cat=_.isUndefined(model.get("settings").category)?"standard":model.get("settings").category;that.categories[cat].modules.push(model)}})},prepareCategories:function(){var cats={};return _.each(KB.fromServer.ModuleCategories,function(item,key){cats[key]={id:key,name:item,modules:[]}}),cats}});var KB=KB||{};KB.Backbone=KB.Backbone||{},KB.Backbone.AreaModel=Backbone.Model.extend({idAttribute:"id"}),KB.Backbone.ModuleDefinition=Backbone.Model.extend({initialize:function(){var that=this;this.id=function(){return"template"===that.get("settings").category?that.get("instance_id"):that.get("settings").class}(),console.log(this)}});var KB=KB||{};KB.Backbone.ModuleMenuTileModel=Backbone.Model.extend({defaults:{template:!1,master:!1,duplicate:!1}});var KB=KB||{};KB.Backbone=KB.Backbone||{},KB.Backbone.ModuleModel=Backbone.Model.extend({idAttribute:"instance_id",destroy:function(){var that=this;KB.Ajax.send({action:"removeModules",instance_id:that.get("instance_id")},that.destroyed)},destroyed:function(){},setArea:function(area){this.area=area},areaChanged:function(){this.view.updateModuleForm()}}),KB.Backbone.ModuleBrowserModuleDescription=Backbone.View.extend({initialize:function(options){this.options=options||{},this.options.browser.on("browser:close",this.close,this)},update:function(){this.$el.empty(),this.$el.html(KB.Templates.render("backend/modulebrowser/module-description",{module:this.model.toJSON()})),this.model.get("settings").helpfile!==!1&&this.$el.append(KB.Templates.render(this.model.get("settings").helpfile,{module:this.model.toJSON()}))},close:function(){}}),KB.Backbone.ModuleBrowserListItem=Backbone.View.extend({tagName:"li",className:"modules-list-item",initialize:function(options){this.options=options||{}},render:function(el){this.model.get("tpldef")?this.$el.html(KB.Templates.render("backend/modulebrowser/module-template-list-item",{module:this.model.toJSON()})):this.$el.html(KB.Templates.render("backend/modulebrowser/module-list-item",{module:this.model.toJSON()})),el.append(this.$el)},events:{click:"loadDetails","click .kb-js-create-module":"createModule"},loadDetails:function(){this.options.parent.trigger("loadDetails",this.model)},createModule:function(){this.options.parent.trigger("createModule",this.model)},close:function(){this.remove()}}),KB.Backbone.ModuleBrowserModulesList=Backbone.View.extend({initialize:function(options){this.options=options||{}},modules:{},subviews:{},setModules:function(modules){return this.modules=modules,this},update:function(){var that=this,first=!1;this.$el.empty(),_.each(this.modules,function(module){that.subviews[module.cid]=new KB.Backbone.ModuleBrowserListItem({model:module,parent:that}),first===!1&&(that.options.browser.loadDetails(module),first=!first),that.$el.append(that.subviews[module.cid].render(that.$el))})}}),KB.Backbone=KB.Backbone||{},KB.Backbone.ModuleBrowser=Backbone.View.extend({initialize:function(options){this.options=options||{}},tagName:"div",id:"module-browser",className:"kb-overlay",events:{"click .close-browser":"close"},subviews:{},render:function(){this.area=this.options.area,this.modulesDefinitions=new KB.Backbone.ModulesDefinitionsCollection(this.prepareAssignedModules(),{model:KB.Backbone.ModuleDefinition}).setup(),this.open()},open:function(){this.$el.appendTo("body"),jQuery("#wpwrap").addClass("module-browser-open"),this.$el.append(KB.Templates.render("backend/modulebrowser/module-browser",{})),this.subviews.ModulesList=new KB.Backbone.ModuleBrowserModulesList({el:jQuery(".modules-list",this.$el),browser:this}),this.subviews.ModuleDescription=new KB.Backbone.ModuleBrowserModuleDescription({el:jQuery(".module-description",this.$el),browser:this}),this.subviews.Navigation=new KB.Backbone.ModuleBrowserNavigation({el:jQuery(".module-categories",this.$el),cats:this.modulesDefinitions.categories}),this.listenTo(this.subviews.Navigation,"browser:change",this.update),this.listenTo(this.subviews.ModulesList,"createModule",this.createModule)},close:function(){jQuery("#wpwrap").removeClass("module-browser-open"),this.trigger("browser:close"),this.unbind(),this.remove(),delete this.$el},update:function(model){var id=model.get("id"),modules=this.modulesDefinitions.getModules(id);this.subviews.ModulesList.setModules(modules).update(),this.listenTo(this.subviews.ModulesList,"loadDetails",this.loadDetails)},loadDetails:function(model){this.subviews.ModuleDescription.model=model,this.subviews.ModuleDescription.update()},createModule:function(module){KB.Checks.userCan("create_kontentblocks")||KB.Notice.notice("You're not allowed to do this","error");var Area=KB.Areas.get(this.options.area.model.get("id"));if(!KB.Checks.blockLimit(Area))return console.log("block limit reached"),KB.Notice.notice("Limit for this area reached","error"),!1;var data={action:"createNewModule","class":module.get("settings").class,master:module.get("master"),master_id:module.get("master_id"),template:module.get("template"),templateObj:module.get("templateObj"),duplicate:module.get("duplicate"),areaContext:this.options.area.model.get("context"),area:this.options.area.model.get("id"),_ajax_nonce:kontentblocks.nonces.create};this.close(),KB.Ajax.send(data,this.success,this)},success:function(data){this.options.area.modulesList.append(data.html),KB.lastAddedModule=new KB.Backbone.ModuleModel(data.module),KB.Modules.add(KB.lastAddedModule),KB.TinyMCE.addEditor(),KB.Fields.trigger("update");var count=parseInt(jQuery("#kb_all_blocks").val(),10)+1;jQuery("#kb_all_blocks").val(count)},prepareAssignedModules:function(){var assignedModules=this.area.model.get("assignedModules"),fullDefs=[];return _.each(KB.fromServer.ModuleDefinitions,function(module){-1!==_.indexOf(assignedModules,module.settings.class)&&fullDefs.push(module)}),fullDefs}}),KB.Backbone.ModuleBrowserNavigation=Backbone.View.extend({item:Backbone.View.extend({initialize:function(options){this.options=options||{}},tagName:"li",className:"cat-item",events:{click:"change"},change:function(){this.options.parent.trigger("browser:change",this.model),this.$el.addClass("active"),jQuery("li").not(this.$el).removeClass("active")},render:function(){var count=_.keys(this.model.get("modules")).length,countstr="("+count+")";return 0===count?!1:(this.options.parent.catSet===!1&&(this.options.parent.catSet=!0,KB.ModuleBrowser.update(this.model),this.$el.addClass("active")),this.options.parent.$list.append(this.$el.html(this.model.get("name")+'<span class="module-count">'+countstr+"</span>")),void 0)}}),catSet:!1,initialize:function(options){var that=this;this.options=options||{},this.$list=jQuery("<ul></ul>").appendTo(this.$el),_.each(this.options.cats,function(cat){var model=new Backbone.Model(cat);new that.item({parent:that,model:model}).render()})}});var KB=KB||{};KB.Backbone=KB.Backbone||{},KB.Backbone.ModuleMenuItemView=Backbone.View.extend({tagName:"div",className:"",isValid:function(){return!0}});var KB=KB||{};KB.Backbone=KB.Backbone||{},KB.Backbone.ModuleDelete=KB.Backbone.ModuleMenuItemView.extend({className:"kb-delete block-menu-icon",initialize:function(){_.bindAll(this,"yes","no")},events:{click:"deleteModule"},deleteModule:function(){KB.Notice.confirm("Really?",this.yes,this.no)},isValid:function(){return this.model.get("predefined")||this.model.get("disabled")||!KB.Checks.userCan("delete_kontentblocks")?!1:!0},yes:function(){KB.Ajax.send({action:"removeModules",_ajax_nonce:kontentblocks.nonces.delete,module:this.model.get("instance_id")},this.success,this)},no:function(){return!1},success:function(){KB.Modules.remove(this.model),wp.heartbeat.interval("fast",2)}});var KB=KB||{};KB.Backbone=KB.Backbone||{},KB.Backbone.ModuleDuplicate=KB.Backbone.ModuleMenuItemView.extend({className:"kb-duplicate block-menu-icon",events:{click:"duplicateModule"},duplicateModule:function(){KB.Ajax.send({action:"duplicateModule",module:this.model.get("instance_id"),areaContext:this.model.area.get("context"),_ajax_nonce:kontentblocks.nonces.create,"class":this.model.get("class")},this.success,this)},isValid:function(){return this.model.get("predefined")||this.model.get("disabled")||!KB.Checks.userCan("edit_kontentblocks")?!1:!0},success:function(data){if(-1===data)return KB.Notice.notice("Request Error","error"),!1;this.model.area.view.modulesList.append(data.html),KB.Modules.add(data.module);var count=parseInt(jQuery("#kb_all_blocks").val(),10)+1;jQuery("#kb_all_blocks").val(count),KB.Notice.notice("Module Duplicated","success"),KB.Ui.repaint("#"+data.module.instance_id)}});var KB=KB||{};KB.Backbone=KB.Backbone||{},KB.Backbone.ModuleStatus=KB.Backbone.ModuleMenuItemView.extend({initialize:function(options){this.options=options||{}},className:"module-status block-menu-icon",events:{click:"changeStatus"},changeStatus:function(){KB.Ajax.send({action:"changeModuleStatus",module:this.model.get("instance_id"),_ajax_nonce:kontentblocks.nonces.update},this.success,this)},isValid:function(){return!this.model.get("disabled")&&KB.Checks.userCan("deactivate_kontentblocks")?!0:!1},success:function(){this.options.parent.$head.toggleClass("module-inactive"),this.options.parent.$el.toggleClass("activated deactivated"),KB.Notice.notice("Status changed","success")}});var KB=KB||{};KB.Backbone=KB.Backbone||{},KB.Backbone.ModuleMenuView=Backbone.View.extend({$menuWrap:null,$menuList:null,initialize:function(){this.$menuWrap=jQuery(".menu-wrap",this.$el),this.$menuWrap.append(KB.Templates.render("be_moduleMenu",{})),this.$menuList=jQuery(".module-actions",this.$menuWrap)},addItem:function(view){if(view.isValid&&view.isValid()===!0){var $liItem=jQuery("<li></li>").appendTo(this.$menuList),$menuItem=$liItem.append(view.el);this.$menuList.append($menuItem)}}});var KB=KB||{};KB.Backbone=KB.Backbone||{},KB.Backbone.AreaView=Backbone.View.extend({initialize:function(){this.controlsContainer=jQuery(".add-modules",this.$el),this.settingsContainer=jQuery(".kb-area-settings-wrapper",this.$el),this.modulesList=jQuery("#"+this.model.get("id"),this.$el),this.model.view=this,this.render()},events:{"click .modules-link":"openModuleBrowser","click .js-area-settings-opener":"toggleSettings"},render:function(){this.addControls()},addControls:function(){this.controlsContainer.append(KB.Templates.render("be_areaAddModule",{}))},openModuleBrowser:function(e){e.preventDefault(),KB.ModuleBrowser=null,KB.ModuleBrowser||(KB.ModuleBrowser=new KB.Backbone.ModuleBrowser({area:this})),KB.ModuleBrowser.render()},toggleSettings:function(e){e.preventDefault(),this.settingsContainer.slideToggle().toggleClass("open"),KB.currentArea=this.model}});var KB=KB||{};KB.Backbone=KB.Backbone||{},KB.Backbone.ModuleView=Backbone.View.extend({$head:null,$body:null,ModuleMenu:null,instanceId:null,events:{"click.kb1 .kb-toggle":"toggleBody","click.kb2 .kb-toggle":"setOpenStatus",mouseenter:"setFocusedModule",dblclick:"fullscreen","click .kb-fullscreen":"fullscreen"},setFocusedModule:function(){KB.focusedModule=this.model},initialize:function(){this.$head=jQuery(".block-head",this.$el),this.$body=jQuery(".kb_inner",this.$el),this.instanceId=this.model.get("instance_id"),this.ModuleMenu=new KB.Backbone.ModuleMenuView({el:this.$el,parent:this}),store.get(this.instanceId+"_open")&&(this.toggleBody(),this.model.set("open",!0)),this.model.view=this,this.setupDefaultMenuItems(),KB.Views.Modules.on("kb:backend::viewDeleted",function(view){view.$el.fadeOut(500,function(){view.$el.remove()})})},setupDefaultMenuItems:function(){this.ModuleMenu.addItem(new KB.Backbone.ModuleDuplicate({model:this.model,parent:this})),this.ModuleMenu.addItem(new KB.Backbone.ModuleDelete({model:this.model,parent:this})),this.ModuleMenu.addItem(new KB.Backbone.ModuleStatus({model:this.model,parent:this}))},toggleBody:function(speed){var duration=speed||400;KB.Checks.userCan("edit_kontentblocks")&&(this.$body.slideToggle(duration),this.$el.toggleClass("kb-open"),KB.currentModule=this.model)},setOpenStatus:function(){this.model.set("open",!this.model.get("open")),store.set(this.model.get("instance_id")+"_open",this.model.get("open")),console.log(this.model.get("open"))},updateModuleForm:function(){KB.Ajax.send({action:"afterAreaChange",module:this.model.toJSON()},this.insertNewUpdateForm,this)},insertNewUpdateForm:function(response){""!==response?this.$body.html(response):this.$body.html("empty"),KB.Ui.repaint(this.$el),this.trigger("kb:backend::viewUpdated")},fullscreen:function(){var that=this;this.sizeTimer=null;var $stage=jQuery("#kontentblocks_stage");$stage.addClass("fullscreen");var $title=jQuery(".fullscreen--title-wrapper",$stage);$title.empty().append("<span class='dashicon fullscreen--close'></span><h2>"+this.model.get("settings").name+"</h2>").show(),jQuery(".fullscreen--close").on("click",_.bind(this.closeFullscreen,this)),this.$el.addClass("fullscreen-module"),jQuery("#post-body").removeClass("columns-2").addClass("columns-1"),this.model.get("open")||(this.setOpenStatus(),this.toggleBody()),this.sizeTimer=setInterval(function(){var h=jQuery(".kb_inner",that.$el).height()+150;$stage.height(h)},750)},closeFullscreen:function(){var $stage=jQuery("#kontentblocks_stage");$stage.removeClass("fullscreen"),clearInterval(this.sizeTimer),this.$el.removeClass("fullscreen-module"),jQuery("#post-body").removeClass("columns-1").addClass("columns-2"),jQuery(".fullscreen--title-wrapper",$stage).hide(),$stage.css("height","100%")}});var KB=KB||{};KB.currentModule={},KB.currentArea={},KB.Views={Modules:new KB.ViewsCollection,Areas:new KB.ViewsCollection,Context:new KB.ViewsCollection},KB.Modules=new KB.Backbone.ModulesCollection([],{model:KB.Backbone.ModuleModel}),KB.Areas=new Backbone.Collection([],{model:KB.Backbone.AreaModel}),KB.App=function(){function init(){KB.Modules.on("add",createModuleViews),KB.Areas.on("add",createAreaViews),KB.Modules.on("remove",removeModule),addViews(),KB.Ui.init()}function addViews(){_.each(KB.fromServer.Areas,function(area){KB.Areas.add(new KB.Backbone.AreaModel(area))}),_.each(KB.fromServer.Modules,function(module){KB.Modules.add(module)})}function createModuleViews(module){module.setArea(KB.Areas.get(module.get("area"))),module.bind("change:area",module.areaChanged),KB.Views.Modules.add(module.get("instance_id"),new KB.Backbone.ModuleView({model:module,el:"#"+module.get("instance_id")})),KB.Ui.initTabs()}function createAreaViews(area){KB.Views.Areas.add(area.get("id"),new KB.Backbone.AreaView({model:area,el:"#"+area.get("id")+"-container"}))}function removeModule(model){KB.Views.Modules.remove(model.get("instance_id"))}return{init:init}}(jQuery),KB.App.init();
=======
